...
# 这里添加所有UI子类 (GetData_UI, AnalyseData_UI, ExportData_UI等)
# ... 保留原有UI子类实现 ...
class GetData_UI(UI):
    def variables(self):
    ...
    def get_data(self):
        ...
        if self.get_data_ui.GetWays_selection.get()=="从本地获取":
            ...
            try:
                ...
                self.get_data_ui.label_GetDataCondition.config(foreground=self.get_data_ui.GetDataCondition_color.get())
                self.urlERR_times+=1
                if self.urlERR_times>=3:
                    open_url("https://www.bilibili.com/video/BV1WwjUzPE6R")
                    self.urlERR_times=0
                    return
            ...
            if response[0]==200:
                ...
                if self.get_data_ui.entry_SaveAsOtherPath.get()!="":
                    try:
                        save_file(self.text,self.get_data_ui.entry_SaveAsOtherPath.get())

                    except PermissionError:
                        self.get_data_ui.GetDataCondition_Add=",另存为时遇到权限错误"
                    except OSError as e:
                        self.get_data_ui.GetDataCondition_Add=",另存为时遇到系统错误:{}".format(e)
                    except Exception as e:
                        self.get_data_ui.GetDataCondition_Add=",另存时遇到未知错误:{}".format(e)

                    else:
                        self.get_data_ui.GetDataCondition_Add='另存成功'
                self.get_data_ui.GetDataCondition.set("状态:获取数据成功"+self.get_data_ui.GetDataCondition_Add)
                if self.get_data_ui.GetDataCondition_Add!="":
                    self.get_data_ui.GetDataCondition_color.set("green")
                    self.get_data_ui.label_GetDataCondition.config(fg=self.get_data_ui.GetDataCondition_color.get())
                else:
                    self.get_data_ui.GetDataCondition_color.set(YELLOW)
                    self.get_data_ui.label_GetDataCondition.config(fg=self.get_data_ui.GetDataCondition_color.get())
